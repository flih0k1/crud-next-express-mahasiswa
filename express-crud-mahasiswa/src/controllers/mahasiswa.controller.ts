...
export const getAllMahasiswa = async (req: Request, res: Response) => {
  try {
    ...
    const [countRows]: any = await db.query(`SELECT COUNT(*) AS total FROM mahasiswa m ${where}`, params);
...
export const createMahasiswa = async (req: Request, res: Response) => {
  try {
    ...
    if (existing.length > 0) return res.status(400).json({ message: "NIM sudah digunakan" });
    ...
    res.status(201).json({ message: "Mahasiswa berhasil ditambahkan", data: { id: result.insertId, nim, nama, prodi_id, angkatan, foto } });
  } catch (error) {
    res.status(500).json({ message: "Terjadi kesalahan server" });
  }
};
...
export const updateMahasiswa = async (req: Request, res: Response) => {
  try {
    ...
    const [result]: any = await db.query(`UPDATE mahasiswa SET ${fields.join(", ")} WHERE id = ?`, values);
