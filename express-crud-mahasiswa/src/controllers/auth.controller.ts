...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    await db.query("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)", [name, email, hashedPassword, "viewer"]);
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    if (!isValidPassword) return res.status(401).json({ message: "Email atau password salah" });

    const token = jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET as string, { expiresIn: (process.env.JWT_EXPIRES_IN || "2h") as any });
